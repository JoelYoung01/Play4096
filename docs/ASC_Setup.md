...
| | |
|---|---|
| Bundle ID (the **app**) | `com.joelyoung.play4096` |
| URL scheme | `play4096://` |
| Pro IAP product (not an app) | `com.joelyoung.play4096.pro` (non-consumable under the app) |
| Native project | generated by `expo prebuild`, never committed |

Do **not** create an App Store Connect *app* whose bundle id is `….pro` — that id is only for the StoreKit product. If an ASC app was created with `.pro` by mistake, leave it alone and let CI create/use a separate app for `com.joelyoung.play4096`.
