...
export function LockScreen() {
  ...
  const tryUnlock = useCallback(async () => { if (prompting) return; setPrompting(true); setFailed(false); try { const ok = await authenticateBiometric(`Unlock ${APP_NAME}`); if (ok) useAppLockStore.getState().unlock(); else setFailed(true); } catch { setFailed(true); } finally { setPrompting(false); } }, [prompting]);
  useEffect(() => { void getBiometricSupport().then((s) => setLabel(s.label)); if (!attempted.current) { attempted.current = true; void tryUnlock(); } }, [tryUnlock]);
